...
If you try to call an external program from your Clojure application, you will
...
If after creating an application using the `customJdk` function, you might see an error like the following:

```
javax.net.ssl.SSLHandshakeException: Received fatal alert: handshake_failure
```

This could be caused by a missing JDK module, such as `jdk.crypto.ec`, which is required for many `https` connections.
Add the missing modules explicitly:

```nix
custom-jdk = pkgs.customJdk {
  cljDrv = clj;
